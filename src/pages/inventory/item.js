...
const ItemPage = ({router}) => {
    ...
    const editItemHandler = (e,nestedKey) => {
        if(nestedKey) {
            return setEditItem({...editItem, Cost: {...editItem.Cost,[e.target.name]:e.target.value}})
        }
    ...
    const deleteItem = (sku) => {
        setLoading(true)
        api.deleteInventory(sku, {"Authorization": `Bearer ${user.accessToken}`})
            .then(data => {
                setLoading(false)
    ...
    return (
        <>
            ...
            {
                showModal && 
                <Modal loading={editItemLoading} title={"Add new product"} closeOnClickOutside={false} onClose={() => modalHandler(false)}>
                    ...
                    <Wrapper padding="22px 0 0" styles={{"width": "632px"}}>
                        <Form onSubmit={submitEditedItem}>
                            <InputGroup>
                                ...
                                <Input wrapperStyles={{"margin-top": "16px", "min-height": "59px", "border": editItemError && !editItem.Name ? "2px solid #CB0000" : ""}} inputStyles={{width: "100%"}} placeholderStyles={{"color": editItemError && !editItem.Name ? "#CB0000" : ""}} placeholder={editItemError && !editItem.Name ? "Required" : "Type"} value={editItem.Name} onChange={(e) => editItemHandler(e)} name="Name" type="text" id="warehouse-new-item-name"/>
                            </InputGroup>
                            <Flex alignItems="flex-start" justifyContent="flex-start" styles={{width: "100%", "margin-top": "24px", gap: "24px"}}>
                                <InputGroup>
                                    ...
                                    <Input wrapperStyles={{"margin-top": "16px", "min-height": "59px"}} inputStyles={{width: "100%"}} placeholder="Type" value={editItem.SKU} onChange={(e) => editItemHandler(e)} name="SKU" type="text" id="warehouse-new-item-sku"/>
                                </InputGroup>
                                <InputGroup>
                                    <Label htmlFor="warehouse-recieving-sku">BARCODE*</Label>
                                    <Input wrapperStyles={{"margin-top": "16px", "min-height": "59px", "border": editItemError && !editItem.Barcode ? "2px solid #CB0000" : ""}} inputStyles={{width: "100%"}} placeholderStyles={{"color": editItemError && !editItem.Barcode ? "#CB0000" : ""}} placeholder={editItemError && !editItem.Barcode ? "Required" : "Type"} value={editItem.Barcode} onChange={(e) => editItemHandler(e)} name="Barcode" type="text" id="warehouse-new-item-barcode"/>
                                </InputGroup>
                            </Flex>
                            <Flex alignItems="flex-start" justifyContent="flex-start" styles={{width: "100%", "margin-top": "24px", gap: "24px"}}>
                                <InputGroup>
                            ...
                            <Wrapper padding="24px 0 0">
                                ...
                                <Flex alignItems="stretch" justifyContent="flex-start" styles={{width: "100%", "margin-top": "16px", gap: "9px"}}>
                                    <InputGroup>
                                        ...
                                        <Input wrapperStyles={{"margin-top": "16px", "min-height": "59px"}} inputStyles={{width: "100%"}} placeholder="0" value={editItem.Cost.ItemCost} onChange={(e) => editItemHandler(e, "Cost")} name="ItemCost" type="number" id="warehouse-new-item-cost"/>
                                    </InputGroup>
                                    <InputGroup>
                                        <ModifiedLabel htmlFor="warehouse-recieving-sku">CUSTOM ENTRY DUTY ($)</ModifiedLabel>
                                        <Input wrapperStyles={{"margin-top": "16px", "min-height": "59px"}} inputStyles={{width: "100%"}} placeholder="0" value={editItem.Cost.CustomEntryDuty} onChange={(e) => editItemHandler(e, "Cost")} name="CustomEntryDuty" type="number" id="warehouse-new-item-ced"/>
                                    </InputGroup>
                                    <InputGroup>
                                        <ModifiedLabel htmlFor="warehouse-recieving-sku">OCEAN FREIGHT ($)</ModifiedLabel>
                                        <Input wrapperStyles={{"margin-top": "16px", "min-height": "59px"}} inputStyles={{width: "100%"}} placeholder="0" value={editItem.Cost.OceanFreight} onChange={(e) => editItemHandler(e, "Cost")} name="OceanFreight" type="number" id="warehouse-new-item-of"/>
                                    </InputGroup>
                                    <InputGroup>
                                        <ModifiedLabel htmlFor="warehouse-recieving-sku">WAREHOUSE DELIVERY ($)</ModifiedLabel>
                                        <Input wrapperStyles={{"margin-top": "16px", "min-height": "59px"}} inputStyles={{width: "100%"}} placeholder="0" value={editItem.Cost.WarehouseDelivery} onChange={(e) => editItemHandler(e, "Cost")} name="WarehouseDelivery" type="number" id="warehouse-new-item-wd"/>
                                    </InputGroup>
                                    <InputGroup>
                                        <ModifiedLabel htmlFor="warehouse-recieving-sku">CUSTOMER SHIPPING ($)</ModifiedLabel>
                                        <Input wrapperStyles={{"margin-top": "16px", "min-height": "59px"}} inputStyles={{width: "100%"}} placeholder="0" value={editItem.Cost.CustomerShipping} onChange={(e) => editItemHandler(e, "Cost")} name="CustomerShipping" type="number" id="warehouse-new-item-cs"/>
                                    </InputGroup>
                                </Flex>
                            </Wrapper>
                            <Wrapper padding="24px 0 0">
                                ...
                                <CustomInput>
                                    ...
                                    <input type="text" name="Tags" value={editItem.TagsInput} onChange={(e) => editItemHandler(e)} />
                                </CustomInput>
                            </Wrapper>
