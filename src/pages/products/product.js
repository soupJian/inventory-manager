...
const ProductPage = ({router}) => {
    ...
    const editProductFieldHandler = (e,nestedKey) => {
        if(nestedKey) {
            return setEditProduct({...editProduct, Cost: {...editProduct.Cost,[e.target.name]:e.target.value}})
        }
        else if(e.target.name === "Tags") {
            let newTags = e.target.value.split(",")
            newTags.pop()
            setEditProduct({...editProduct, TagsInput:e.target.value, Tags:newTags})
        }
        else {
    ...
    const deleteProduct = (sku) => {
        setLoading(true)
        api.deleteProduct(sku, {"Authorization": `Bearer ${user.accessToken}`})
            .then(data => {
                setLoading(false)
    ...
    const submitEditedProduct = (e) => {
        ...
        if(!editProduct.Name) {
            ...
            let data = {...editProduct, Updated: new Date(), Created: new Date(), Parts}
                ...
                if(data.Items[0]?.Parts.length) {
                    ...
                    .then(items => { 
                        const partsInputData = items.Items?.map(item => {
                            const count = data.Items[0]?.Parts?.filter((part) => part.SKU === item.SKU)[0]?.Quantity
    ...
    return (
        <>
            ...
            {
                showModal && 
                <Modal loading={editProductLoading} title={"Add new product"} closeOnClickOutside={false} onClose={() => modalHandler(false)}>
                    ...
                    <Wrapper padding="22px 0 0" styles={{"width": "632px"}}>
                        <Form onSubmit={submitEditedProduct}>
                            <Wrapper padding="0 0 0">
                                ...
                                <Button type="button" onClick={addPart} minWidth="170px" styles={{"gap": "6px", "margin-top": "24px"}} kind="primary">
                            ...
                            <Wrapper padding="24px 0 0">
                                <InputGroup>
                                    ...
                                    <Input wrapperStyles={{"margin-top": "16px", "min-height": "59px"}} inputStyles={{width: "100%"}} placeholder="Type" value={editProduct.Name} onChange={(e) => editProductFieldHandler(e)} name="Name" type="text" id="products-new-product-name"/>
                                </InputGroup>
                            </Wrapper>
                            <Wrapper padding="24px 0 0">
                                <InputGroup>
                                    <Label htmlFor="products-new-product-sku">SKU</Label>
                                    <Input wrapperStyles={{"margin-top": "16px", "min-height": "59px"}} inputStyles={{width: "100%"}} placeholder="Type" value={editProduct.SKU} onChange={(e) => editProductFieldHandler(e)} name="SKU" type="text" id="products-new-product-sku"/>
                                </InputGroup>
                            </Wrapper>
                            <Wrapper padding="24px 0 0">
                                ...
                                <CustomInput>
                                    ...
                                    <input type="text" name="Tags" value={editProduct.TagsInput} onChange={(e) => editProductFieldHandler(e)} />
                                </CustomInput>
                            </Wrapper>
