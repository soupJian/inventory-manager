...
const SearchPage = ({router}) => {
    ...
    const [searchEntity, setSearchEntity] = useState(router.query.searchEntity || "")
    const [search, setSearch] = useState(router.query.search || "")
    const [data, setData] = useState({})
    const [loadingData, setLoadingData] = useState(false)
    const [loadingTable, setLoadingTable] = useState(false)
    const [selection, setSelection] = useState([])
    const [status, setStatus] = useState(router.query.status ? router.query.status.split(",") : [])
    ...
    const clearSelectedItems = () => {
        if(searchEntity === "Inventory") {
            ...
            const itemsToBeCleared = data.Items.filter(item => selection.includes(item.SKU))
            Promise.all(
    ...
    const deleteSelectedItems = () => {
        ...
        if(searchEntity === "Inventory") {
            ...
            Promise.all(
                selection.map((item) => {
                    return api.deleteProduct(item, {"Authorization": `Bearer ${user.accessToken}`})
                })
            )
            .then(values => {
                setLoadingTable(false)
    ...
    useEffect(() => {
        if(searchEntity !== router.query.searchEntity || search !== router.query.search) {
    ...
    return (
        <Wrapper styles={{"min-height": "100%", "position": "relative", display: "flex", "flex-direction": "column", "align-items": "stretch"}} height="auto" padding="21px 29px">
            {
                loadingData && 
                <LoadingWrapper>
                    <Loader size={100} />
            ...
            <Flex alignItems="center" justifyContent="flex-end" styles={{flex: "0 0 auto","margin-top": "27px", gap: "12px"}}>
                <Filter value={status} label="Status" list={statusList} multiSelect onSelect={handleStatus} />
            </Flex>
            <Wrapper styles={{position: "relative", flex: "1 0 auto"}} padding="23px 0px 0px">
                {   !loadingData &&
                    !data.Items?.length ?
                        <Flex alignItems="flex-start" justifyContent="center" styles={{width: "100%", height:"100%", "padding-top": "66px"}}>
                            <Box 
                                borderType="solid" 
                            ...
                            >
                                ...
                                <Text color="#000000" size="18px">No results found</Text>
                                <Text color="#999999" size="15px">Adjust your search and try again</Text>
                            </Box>
                        </Flex>
                        :
                        <>
                            {
                                ...
                                <Table
                                    ...
                                    onSelectAll={selectAll} headers={ItemTableHeaders}
                                    >
                                    {
