  ...
  return (
    <UnitWrapper padding="0">
      <Flex alignItems="center" justifyContent="flexstart" gap="16px">
        <Line />
        <Text size="18px" color="#000000" family="Rubik" weight="600">
          Unit {unit}
        </Text>
        <Text size="18px" color="#000000" family="Lato" weight="500">
          {label}
        </Text>
      </Flex>
      <Wrapper
        styles={{ position: 'relative', 'margin-top': '24px', width: 'auto' }}
        padding="22px 28px 0 0"
      >
        <XLabel>
          <Flex alignItems="center" justifyContent="space-around">
            {xLabel?.map((label) => (
              <Text key={label} color="#999999" family="Rubik" size="14px">
                {label}
              </Text>
            ))}
          </Flex>
